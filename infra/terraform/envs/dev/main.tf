...
terraform {
  ...
  backend "gcs" {
    bucket = "resume-gen-intent-dev-tf-state"
    prefix = "dev"
  }
}
...
variable "project_id" {
  ...
  validation {
    ...
    error_message = "GUARDRAIL: project_id must be 'resume-gen-intent-dev'. No other projects allowed without explicit approval."
  }
}
...
variable "region" {
  ...
  validation {
    ...
    error_message = "GUARDRAIL: region must be 'us-central1'. Multi-region deployment not enabled without explicit approval."
  }
}
